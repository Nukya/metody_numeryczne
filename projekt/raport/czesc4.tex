...
Celem czwartej części projektu było wyznaczenie minimalnej masy oleju chłodzącego, która pozwala schłodzić pręt do temperatury $125^\circ$C w czasie $t=0{.}7$ s. W tym celu wykorzystano pełny, nieliniowy model chłodzenia wraz z aproksymowaną funkcją współczynnika wnikania ciepła $h(\Delta T)$ otrzymaną z funkcji sklejanych kubicznych. Rozwiązanie równania $T_b(m_w, 0{.}7\ \mathrm{s}) = 125^\circ\mathrm{C}$ przeprowadzono metodą Newtona--Raphsona.
...
\begin{table}[h!]
	\centering
	\caption{Iteracje metody Newtona-Raphsona dla równania $T_b(m_w)=125^\circ$C.}
	\label{tab:newton}
	\begin{tabular}{c|c|c|c|c}
		...
		$T_b(m_w^{(i)})$ [$^\circ$C] &
		$T_b'(m_w^{(i)})$ [$^\circ$C/kg] &
		$\Delta T_b = T_b - T_\mathrm{cel}$ [$^\circ$C] \\
		\hline
